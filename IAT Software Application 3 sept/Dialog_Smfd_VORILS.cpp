...
//11.01 TACAN Channel 12, X-mode	Air-to-Ground	0x460C21A6
		...
		if(g_bSimulationMode) 
		{
			...
			ReadVorIlsTacanRX(8);
			 ...
			 if (bVorPhase)
            {
				if (GetARINCValue(8, LABEL_222, &value))
					m_edtvorbrg = (int)value;

				if (GetARINCValue(8, LABEL_034, &value))
					 m_chk_SMFD_VorFreq = (value == 0.0f) ? TRUE : FALSE;   // 0 = VOR-tuned confirmed
			}
			 else
            {
                ...
                if (GetARINCValue(8, LABEL_173, &value))
                {
                    CString str;
                    str.Format(_T("%.3f"), value);
                    m_edt_LocDev173 = str;
                ...
                if (GetARINCValue(8, LABEL_174, &value))
                {
                    CString str;
                    str.Format(_T("%.3f"), value);
                    m_edt_GlsDev174 = str;
