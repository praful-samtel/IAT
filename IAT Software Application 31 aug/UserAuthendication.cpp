...
void CUserAuthendication::OnLogin() 
{
	...
	int iFileRetVal = 0;
	unsigned short usLoopIndex = 0,usUserFlag = 0;
	...
	if(m_TxtUserName == "Administrator" && m_TxtEditPassword == "admin")
	{
	...
	char carrSysDir[1024], carrUserDetailsFilePath[1024];
	...
	if(size == 0)
	{
		...
		//Encode the user names and Passwords before writing into the file
		for(usLoopIndex = 0;usLoopIndex < ulNumberOfUsers;usLoopIndex++)
		{
			...
			//Write the Encoded structure into the File
	...
	else
	{
		...
		for(usLoopIndex = 0;usLoopIndex < ulNumberOfUsers;usLoopIndex++)
		{
			...
			{
				usUserFlag = 0;
				if(UserDetailsDecoded.uc_Type==1)
					g_cAdminstrator = 1;
				else
					g_cAdminstrator = 2;
				break;
		...
		if(m_TxtEditPassword == UserDetailsDecoded.arr_cPassword)
		{
			fclose(pfUserDetails);
