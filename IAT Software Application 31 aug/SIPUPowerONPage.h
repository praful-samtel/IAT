...
extern CString          g_strSelfTestFileName;   // path of the current self-test .bin, mirrors g_strFileName
extern sSelfTest_Header s_SelfTest_IndexPage;     // header record, mirrors s_Index_Page
extern sSelfTest_Item   s_Curr_SelfTest_Item;     // scratch record, mirrors s_Curr_Test_Result
...
//protected:
	public:
		...
		void SetTestExecutionControls(BOOL bEnable);
		void ExecuteDUConfig2Sequence();
		void UpdateListBoxData();
		void ExecuteBothSMFD();
		void ProcessTest(
    int iCurrentTest,
    int& bDlgFlag,
    int& iResult,
 ...
 afx_msg void OnCbnSelchangeComboSMFDMode();
