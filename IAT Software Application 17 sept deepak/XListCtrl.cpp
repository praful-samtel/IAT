...
int CXListCtrl::InsertItem(const LVITEM* pItem)
{
	...
	// FIX: Ensure we have columns before allocating
	int nColumns = GetColumns();
	if (nColumns <= 0)
		nColumns = 1;  // At least allocate for 1 column

	XLISTCTRLDATA *pXLCD = new XLISTCTRLDATA[nColumns];
	ASSERT(pXLCD);
	if (!pXLCD)
		return -1;
	...
	for (int i = 0; i < nColumns; i++)
	{
		...
		pXLCD[i].nImage       = (i == 0) ? pItem->iImage : -1;
		pXLCD[i].nCheckedState = -1;
		pXLCD[i].bEnabled     = TRUE;
		pXLCD[i].bBold        = FALSE;
		pXLCD[i].bShowProgress = FALSE;
		pXLCD[i].nProgressPercent = 0;
		pXLCD[i].bShowProgressMessage = FALSE;
		pXLCD[i].bCombo       = FALSE;
		pXLCD[i].bComboIsClicked = FALSE;
		pXLCD[i].nComboListHeight = 0;
		pXLCD[i].nInitialComboSel = 0;
		pXLCD[i].psa          = NULL;
		pXLCD[i].dwItemData   = 0;
		pXLCD[i].strToolTip   = _T("");
	}
						   ...
						   COLORREF crBackground)
{
	...
	for (int i = 0; i < nColumns; i++)
	{
		pXLCD[i].crText       = (i == 0) ? crText : m_crWindowText;
		pXLCD[i].crBackground = (i == 0) ? crBackground : m_crWindow;
		pXLCD[i].nImage       = -1;
		pXLCD[i].nCheckedState = -1;
		pXLCD[i].bEnabled     = TRUE;
		pXLCD[i].bBold        = FALSE;
...
//updated it for 64 bit
#if 0 // updated it for 64 bit
BOOL CXListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText,
					...
					COLORREF crText, COLORREF crBackground)
{
	...
	// FIX: Add proper validation and reallocation if needed
	XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(nItem);
	
	// Check if pointer is valid (not NULL and not -1)
	if (pXLCD && pXLCD != (XLISTCTRLDATA*)-1)
	{
		// Additional check: verify this looks like a valid pointer
		// by trying to access it in a try-catch block
		try
		{
			// Test access before using
			COLORREF testColor = pXLCD[0].crText;  // Test read
			
			// If we get here, pointer seems valid
			// Now check if we need to reallocate for more columns
			int nColumns = GetColumns();
			
			// Note: We can't easily know the original allocation size,
			// so we'll just be careful with the access
		...
		{
			// Pointer is invalid, don't set colors
			TRACE("Invalid XLISTCTRLDATA pointer for item %d\n", nItem);
			// Optionally, allocate new data here
			return rc;  // Return result of SetItemText
		}
	}
	else if (!pXLCD)
	{
		...
		XLISTCTRLDATA *pNewXLCD = new XLISTCTRLDATA[nColumns];
		if (pNewXLCD)
		{
			// Initialize all elements
			for (int i = 0; i < nColumns; i++)
			{
				pNewXLCD[i].crText       = m_crWindowText;
				pNewXLCD[i].crBackground = m_crWindow;
				pNewXLCD[i].nImage       = -1;
				pNewXLCD[i].nCheckedState = -1;
				pNewXLCD[i].bEnabled     = TRUE;
				pNewXLCD[i].bBold        = FALSE;
				pNewXLCD[i].bShowProgress = FALSE;
				pNewXLCD[i].nProgressPercent = 0;
				pNewXLCD[i].bShowProgressMessage = FALSE;
				pNewXLCD[i].bCombo       = FALSE;
				pNewXLCD[i].bComboIsClicked = FALSE;
				pNewXLCD[i].nComboListHeight = 0;
				pNewXLCD[i].nInitialComboSel = 0;
				pNewXLCD[i].psa          = NULL;
				pNewXLCD[i].dwItemData   = 0;
				pNewXLCD[i].strToolTip   = _T("");
			}
			
			// Set the requested colors
			pNewXLCD[nSubItem].crText       = (crText == -1) ? m_crWindowText : crText;
			pNewXLCD[nSubItem].crBackground = (crBackground == -1) ? m_crWindow : crBackground;
			
			// Store the new data
			CListCtrl::SetItemData(nItem, (DWORD_PTR)pNewXLCD);
		}
	}
