...
extern CSerialPort gCommPort12,gCommPort1;
...
void CBenchSelfTest::DoPWRSupply()
{
	...
	Sleep(500);
    // Read output voltage
    double dVoltage = ReadPowerSupplyValue('V');

    // Check for approximately 28V
    if (dVoltage >= 27.0 && dVoltage <= 29.0)
    {
        m_chkPwrSupply = TRUE;
    ...
    {
        m_chkPwrSupply = FALSE;
        GetDlgItem(IDC_CHECK_PWRSPLY)->SetWindowText("FAIL");
    }
	Off28Power("com12");
    UpdateData(FALSE);
}
