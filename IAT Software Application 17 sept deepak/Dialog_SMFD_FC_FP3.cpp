...
BEGIN_MESSAGE_MAP(CDialog_SMFD_FC_FP3, CDialogEx)
    ...
    ON_BN_CLICKED(
        IDC_BUTTON_SMFD_ANALOG_FCFP3,
        &CDialog_SMFD_FC_FP3::OnBnClickedButtonSmfdAnalogFcfp3
    )

    ON_BN_CLICKED(
        IDOK,
        &CDialog_SMFD_FC_FP3::OnBnClickedOk
    )

    ON_MESSAGE(
        WM_UPDATE_SMFD_ANALOG,
        &CDialog_SMFD_FC_FP3::OnUpdateSMFDAnalog
    )
...
void CDialog_SMFD_FC_FP3::OnBnClickedButtonSmfdAnalogFcfp3()
{
    ...
    // Generate Flap 3V voltage
    if (!g_AnalogOutput.SetVoltage(6, 3.00))
    {
        AfxMessageBox(
            _T("Failed to generate Flap 3V voltage.")
        );
...
LRESULT CDialog_SMFD_FC_FP3::OnUpdateSMFDAnalog(
    WPARAM wParam,
    LPARAM lParam
)
{
...
void CDialog_SMFD_FC_FP3::OnDestroy()
{
