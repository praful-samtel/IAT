...
LRESULT CDialog_Smfd_VORILS::OnUpdateArinc(WPARAM wParam, LPARAM lParam)
{
	UpdateData(FALSE);
	return 0;
}
...
//11.01 TACAN Channel 12, X-mode	Air-to-Ground	0x460C21A6
		...
		if(g_bSimulationMode) 
		{
			 ...
			 if (bVorPhase)
            {
				...
				{
					 m_chk_SMFD_VorFreq = (value == 0.0f) ? TRUE : FALSE;   // 0 = VOR-tuned confirmed
					TRACE("m_chk_SMFD_VorFreq ::: %d values :: %d ",m_chk_SMFD_VorFreq,(float)value);

				}
				if (GetARINCValue(9, LABEL_222, &value))
				{
					m_edtvorbrg = (int)value;
					TRACE("m_edtvorbrg ::: %d values :: %d ",m_edtvorbrg,(int)value);
				}
			 ...
			 else
            {
				...
				{
                    m_chk_SMFD_IlsFreq = (value == 1.0f) ? TRUE : FALSE;   // 1 = ILS-tuned confirmed
					TRACE("m_chk_SMFD_IlsFreq ::: %d values :: %d ",m_chk_SMFD_IlsFreq,(float)value);
				}
                if (GetARINCValue(9, LABEL_173, &value))
                {
                    ...
                    str.Format(_T("%.3f"), value);
                    m_edt_LocDev173 = str;
					TRACE("m_edt_LocDev173 ::: %d values :: %d ",m_edt_LocDev173,(float)value);
                }
                ...
                if (GetARINCValue(9, LABEL_174, &value))
                {
                    ...
                    str.Format(_T("%.3f"), value);
                    m_edt_GlsDev174 = str;
					TRACE("m_edt_GlsDev174 ::: %d values :: %d ",m_edt_GlsDev174,(float)value);
                }
            }
