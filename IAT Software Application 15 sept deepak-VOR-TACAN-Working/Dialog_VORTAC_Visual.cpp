...
void CDialog_VORTAC_Visual::OnBnClickedOk()
{
	...
	{
		g_strMsgStr = "VOR Receiver not entered Correct. Continue?";
		msgdlg.DoModal();
	...
	{
		g_strMsgStr = "TACAN Receiver not entered Correct. Continue?";
		msgdlg.DoModal();
		if (!g_bContinueFlag) return;
	...
	{
		g_strMsgStr = "CDU not entered Correct. Continue?";
		msgdlg.DoModal();
		if (!g_bContinueFlag) return;
