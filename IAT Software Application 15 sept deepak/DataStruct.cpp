...
//        memcpy(&packet, buff, sizeof(packet));
//        if (packet.SSM != 0x03) return;   // Normal only
...
//
//    default:
//        break;   // LABEL_034, LABEL_145: presence/parity only, no numeric decode needed here
//    }
//}

void DecodeGpsArincData(unsigned int buff[4],
                        unsigned int* plabel,
                        float* pdata)
{
    unsigned int word = buff[0];

    ARINC_PACKET headerPeek;
    memcpy(&headerPeek, buff, sizeof(headerPeek));
    *plabel = headerPeek.label;

    // GPS ICD: SSM is documented as "Not Used", fixed at 00 for every
    // label here -- deliberately NOT gating on SSM the way ADC/AHRS/VOR/
    // TACAN decode paths do.

    switch (*plabel)
    {
    case LABEL_012:   // GPS Ground Speed, 4 BCD digits, bits 9-24
    {
        unsigned int d1 = (word >> 8)  & 0xF;   // ones
        unsigned int d2 = (word >> 12) & 0xF;   // tens
        unsigned int d3 = (word >> 16) & 0xF;   // hundreds
        unsigned int d4 = (word >> 20) & 0xF;   // thousands
        *pdata = (float)(d4*1000 + d3*100 + d2*10 + d1);
        break;
    }

    case LABEL_013:   // GPS True Track, 4 BCD digits (0.1/1/10/100 deg), bits 9-24
    case LABEL_053:   // GPS Mag Track -- identical layout, different label
    {
        unsigned int tenths = (word >> 8)  & 0xF;
        unsigned int ones_d = (word >> 12) & 0xF;
        unsigned int tens_d = (word >> 16) & 0xF;
        unsigned int hund_d = (word >> 20) & 0xF;
        *pdata = hund_d*100.0f + tens_d*10.0f + ones_d + tenths*0.1f;
        break;
    }

    case LABEL_125:   // GPS Time HH:MM:SS -- mixed 4-bit/3-bit/2-bit fields
    {
        unsigned int sec1  = (word >> 8)  & 0xF;
        unsigned int sec10 = (word >> 12) & 0x7;
        unsigned int min1  = (word >> 15) & 0xF;
        unsigned int min10 = (word >> 19) & 0x7;
        unsigned int hr1   = (word >> 22) & 0xF;
        unsigned int hr10  = (word >> 26) & 0x3;
        // Encode HHMMSS into *pdata as a single number (e.g. 120000 for
        // 12:00:00) since GetARINCValue only carries a float -- the caller
        // (OnTimer/RxThread) should format it back into HH:MM:SS for display.
        *pdata = (float)((hr10*10+hr1)*10000 + (min10*10+min1)*100 + (sec10*10+sec1));
        break;
    }
...
BOOL ReadGpsRX(ADT_L0_UINT32 ChannelNo)
{
    ADT_L0_UINT32 status;
    ADT_L0_UINT32 numRXPs;
    ADT_L1_A429_RXP myRXP_buffer[100];
