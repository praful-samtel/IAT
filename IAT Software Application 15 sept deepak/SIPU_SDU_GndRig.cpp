...
BOOL CSIPU_SDU_GndRigApp::InitInstance()
{
	...
	BOOL PCI1824 = g_AnalogOutput.Initialize();
	if(PCI1824)
	{
		TRACE("1824 init Success");
	}else{
		TRACE("1824 init Failed");
	}
...
void CSIPU_SDU_GndRigApp::OnTerminateTestSeq()
{
	...
	// Leave the bench in a safe state before returning control to the operator.
	BOOL bDiscretesOff = DisableAllLRUPowerDiscretes();

	// Switch off power supply if COM12 is available
	BOOL bPowerOff = SwitchOFFPowerSupply();
	BOOL bArincClosed = CloseARINC429();
	 g_AnalogOutput.Close();
	// set the stop flag
	m_bTestStopFlag = true;
	TRACE("[STOP] Safety shutdown finished: discretes=%s, 28V=%s, ARINC=%s.\n",
		bDiscretesOff ? "OFF" : "FAILED",
		bPowerOff ? "OFF" : "FAILED",
		bArincClosed ? "CLOSED" : "FAILED");
}
...
BOOL CSIPU_SDU_GndRigApp::SwitchOFFPowerSupply(void)
{
	BOOL bTrsFlag;
	BYTE ucWriteData;
	BYTE ucReadData;
	...
	if (!gCommPort12.IsOpen())
	{
		if (!InitializePowerSupplyPort())
		{
			TRACE("Unable to open COM12 while switching off the power supply.\n");
			return FALSE;
		}
	}
	...
	{
		TRACE("[STOP] 28V power supply OFF command was not acknowledged. Response='%s'\n", response);
		return FALSE;
	}

	TRACE("[STOP] 28V power supply is OFF.\n");
	return TRUE;
}
