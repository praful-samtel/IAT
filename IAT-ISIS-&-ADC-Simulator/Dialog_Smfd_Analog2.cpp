...
void CDialog_Smfd_Analog2::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_HYDPRESEMER, m_edt_hydpresemer);
	DDX_Text(pDX, IDC_EDIT_ACCUMPRES, m_edt_emerbrake);
	DDX_Text(pDX, IDC_EDIT_LHBRAKE, m_edt_Lhbrake);
	DDX_Text(pDX, IDC_EDIT_RHBRAKE, m_edt_rhbrake);
	DDX_Text(pDX, IDC_EDIT_ENGSPD, m_edt_enginespd);
	DDX_Text(pDX, IDC_EDIT_LHFUEL, m_edt_fuelLh);
	DDX_Text(pDX, IDC_EDIT_RHFUEL, m_edt_fuelRh);
	DDX_Text(pDX, IDC_EDIT_COLLECTORANK, m_edt_collectortank);
}
...
void CDialog_Smfd_Analog2::OnBnClickedOk()
{
    ...
    // -------- HYD PRES EMER --------
    val = atoi(m_edt_hydpresemer);
    if (val >= 3310 && val <= 3350)
    {
        m_strHydPresEmer = m_edt_hydpresemer;
...
void CDialog_Smfd_Analog2::OnBnClickedButtonSmfdAnalog()
{
	...
	UpdateData(TRUE);
	    ...
	    m_edt_hydpresemer = "3340";   // 3330± 20 psi
		m_edt_emerbrake = "1307";   // 1300± 15 psi
		m_edt_Lhbrake = "1205"; //1200± 15 psi
		m_edt_rhbrake = "1205"; //1200± 15 psi
		m_edt_enginespd = "80"; //79±2 % rpm
		m_edt_fuelLh = "170"; //168±5 lb
		m_edt_fuelRh = "162"; //168±5 lb
		m_edt_collectortank = "185";  //183±5 lb
