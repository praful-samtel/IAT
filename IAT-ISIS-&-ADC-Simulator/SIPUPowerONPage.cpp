...
void CSIPUPowerONPage::OnButtonExec() 
{
	...
	GetDlgItem(IDC_BUT_PRINT) ->EnableWindow(false);

		ExecuteLRUTestSequence();

		if (pMainFrame->m_bTestStopFlag)
		{
			AfxMessageBox("DU Test Sequence Execution Terminated");
		}
		else
		{
			//AfxMessageBox("DU Test Sequence Execution Completed");
		}
		// CloseARINC429();
		pMainFrame->m_bTestStopFlag = false;
		//g_bMFDPowerONStatus = false;
		
	GetDlgItem(IDC_BUTTON_EXEC) ->EnableWindow(true);
	GetDlgItem(IDC_BUT_PRINT) ->EnableWindow(true);
...
if(mode == MODE_INTEGRATION)
{
    ...
    ExecuteISISSequence(&fptr);
    ExecuteADCSequence(&fptr);
    ExecuteAHRSTestSequence(&fptr);
    ExecuteGPSTestSequence(&fptr);
    ExecuteVORILSTACTestSequence(&fptr);
...
else
{
    ...
    if(iSelectedLRU == 0)        // SMFD
    {
		strcpy(s_Index_Page.m_strEquipment,"SMFD");
     strcpy(s_Index_Page.m_strSmfdFc,m_strSMFD1SNO);
	 strcpy(s_Index_Page.m_strSmfdRc,m_strSMFD2SNO);
	 strcpy(s_Index_Page.m_strMfdFc,m_strMFD1SNO);
	 strcpy(s_Index_Page.m_strMfdRc,m_strMFD2SNO);
    ...
    { 
		strcpy(s_Index_Page.m_strEquipment,"VOR");
		strcpy(s_Index_Page.m_strVorrx,m_strNAV_SNO);
	strcpy(s_Index_Page.m_strCdu,m_strNAV_SNO);
	 strcpy(s_Index_Page.m_strTacrx,m_strNAVTAC_SNO);
    strcpy(s_Index_Page.m_strFileName,g_chFileName);
...
/*
    ...
    if (dlg.DoModal() == IDCANCEL)
    {
        pMainFrame->m_bTestStopFlag = TRUE;
        return;
    }
    ...
    if (!fptr.Open(g_chFileName, CFile::typeBinary | CFile::modeWrite | CFile::modeCreate))
    {
        pMainFrame->m_bTestStopFlag = TRUE;
        return;
    }
    ...
    strcpy(s_Index_Page.m_strOperator, (LPCTSTR)m_edtStrTestDesc);
...
if(pUseFile == NULL)
{
	...
	//pFile->Write(&s_Curr_Test_Result, sizeof(s_Curr_Test_Result));
    // ===== DIALOG OBJECTS =====
    ...
    // ===== TEST TABLE (ISIS → 2.xx → 201–217) =====
#define ADD_TEST(maj, id, majName, minName, dlgPtr, exp) \
    s_AllTestDetails[iTempCount].m_ucMajor_Test_ID = maj; \
    s_AllTestDetails[iTempCount].m_uiMinor_Test_ID = id; \
    ...
    // VISUAL (201–205)
    ADD_TEST(2,201,"Visual Inspection","Display Unit FC",&m_Dlg_Visual_Inspctn,"IS2100000000");
	ADD_TEST(2,202,"Visual Inspection","Display Unit RC",&m_Dlg_Visual_Inspctn,"IS2100000000");
    ADD_TEST(2,203,"Visual Inspection","Air Data Unit",&m_Dlg_Visual_Inspctn,"AN-014-ADU-V00");
    ADD_TEST(2,204,"Visual Inspection","Magnetometer",&m_Dlg_Visual_Inspctn,"AN-015-MMS-V00");
    ADD_TEST(2,205,"Visual Inspection","AHRS",&m_Dlg_Visual_Inspctn,"1901400000000");
    ADD_TEST(2,206,"Visual Inspection","Any Damages",&m_Dlg_Visual_Inspctn,"NO");
	ADD_TEST(2,207,"Visual Inspection","Visible Scratches",&m_Dlg_Visual_Inspctn,"NO");
	ADD_TEST(2,208,"Visual Inspection","Loose Screws",&m_Dlg_Visual_Inspctn,"NO");
	ADD_TEST(2,209,"Visual Inspection","Bend Pins",&m_Dlg_Visual_Inspctn,"NO");
    // POWER
    ADD_TEST(2,210,"Power","Current Rating ISIS-DU FC",&m_Dlg_ISIS_PWR,"< 1.08 A");
	ADD_TEST(2,211,"Power","Current Rating ISIS-DU RC",&m_Dlg_ISIS_PWR,"< 1.08 A");
	ADD_TEST(2,212,"Power","Current Rating ISIS ADU",&m_Dlg_ISIS_PWR,"< 0.3 A");
	ADD_TEST(2,213,"Power","Current Rating ISIS AHRS",&m_Dlg_ISIS_PWR,"< 1 A");

    // SOFTWARE
    ADD_TEST(2,214,"Software","ISIS-DU FC MPM Version",&m_Dlg_ISIS_SWCHK,"01.00");
    ADD_TEST(2,215,"Software","ISIS-DU FC MPM Checksum",&m_Dlg_ISIS_SWCHK,"0xCA347B86");
    ADD_TEST(2,216,"Software","ISIS-DU FC BLGM Version",&m_Dlg_ISIS_SWCHK,"01.00");
    ADD_TEST(2,217,"Software","ISIS-DU FC BLGM Checksum",&m_Dlg_ISIS_SWCHK,"0x43760D0A");
	 ADD_TEST(2,218,"Software","ISIS-DU RC MPM Version",&m_Dlg_ISIS_SWCHK,"01.00");
    ADD_TEST(2,219,"Software","ISIS-DU RC MPM Checksum",&m_Dlg_ISIS_SWCHK,"0xCA347B86");
    ADD_TEST(2,220,"Software","ISIS-DU RC BLGM Version",&m_Dlg_ISIS_SWCHK,"01.00");
    ADD_TEST(2,221,"Software","ISIS-DU RC BLGM Checksum",&m_Dlg_ISIS_SWCHK,"0x43760D0A");

	//IBIT Checks 
	ADD_TEST(2,222,"IBIT Checks","ISIS-DU FC",&m_Dlg_ISIS_IBIT,"Operational");
    ADD_TEST(2,223,"IBIT Checks","ISIS-DU RC",&m_Dlg_ISIS_IBIT,"Operational");

	//brightness  Checks 
	ADD_TEST(2,224,"Brightness and Set knob Checks","ISIS-DU FC",&m_Dlg_ISIS_BRT,"Operational");
    ADD_TEST(2,225,"Brightness and Set knob Checks","ISIS-DU RC",&m_Dlg_ISIS_BRT,"Operational");

	//brightness  Checks 
	ADD_TEST(2,226,"Alignment mode Checks","ISIS-DU FC",&m_Dlg_ISIS_ALN,"Operational");
    ADD_TEST(2,227,"Alignment mode Checks","ISIS-DU RC",&m_Dlg_ISIS_ALN,"Operational");

	//brightness  Checks 
	ADD_TEST(2,228,"Navigation mode Checks","ISIS-DU FC",&m_Dlg_ISIS_NAV,"Operational");
    ADD_TEST(2,229,"Navigation mode Checks","ISIS-DU RC",&m_Dlg_ISIS_NAV,"Operational");

    // AHRS
    ADD_TEST(2,230,"AHRS","Roll",&m_Dlg_ISIS_AHRS,"OK");
    ADD_TEST(2,231,"AHRS","Pitch",&m_Dlg_ISIS_AHRS,"OK");
    ADD_TEST(2,232,"AHRS","Heading",&m_Dlg_ISIS_AHRS,"OK");

    // ADU
    ADD_TEST(2,233,"ADU","Airspeed",&m_Dlg_ISIS_ADU,"192±5");
    ADD_TEST(2,234,"ADU","Altitude",&m_Dlg_ISIS_ADU,"9980±30");
    ADD_TEST(2,235,"ADU","Mach",&m_Dlg_ISIS_ADU,"0.37± 0.012 M");
    ADD_TEST(2,236,"ADU","Vertical Speed",&m_Dlg_ISIS_ADU,"Satisfactory");
    ...
    if(iCurrentTest % 100 <= DU_ATP_MAX_NO_OF_SUBTEST)
    {
        for(iCurr_Pointer = 0; iCurr_Pointer < iTempCount; iCurr_Pointer++)
        {
            if(iCurrentTest == s_AllTestDetails[iCurr_Pointer].m_uiMinor_Test_ID)
            {
                ...
                // ================= GROUP 2 (ISIS VISUAL + POWER + SW + AHRS + ADU) =================

                // -------- VISUAL (201–205) --------
                if ((iCurrentTest / 100) == 2)
                {
                    if (!bDlgFlag)
                    {
                        iResult = s_AllTestDetails[iCurr_Pointer].m_Procedure_Dlg->DoModal();
                        bDlgFlag = TRUE;
                    }
                    ...
                    case 236:
                        strcpy(s_Curr_Test_Result.m_strMeasuered_Val, m_Dlg_ISIS_ADU.m_strISISVertical);
                        strcpy(s_Curr_Test_Result.m_strTestStatus,  m_Dlg_ISIS_ADU.m_strStatusISISVertical);
...
#define ADD_TEST(idInteg, idStandalone, majStandalone, majName, minName, dlgPtr, exp) \
    ...
    ADD_TEST(201,101,1,"Visual Inspection","Display Unit FC",&m_Dlg_Visual_Inspctn,"IS2100000000");
    ADD_TEST(202,102,1,"Visual Inspection","Display Unit RC",&m_Dlg_Visual_Inspctn,"IS2100000000");
    ADD_TEST(203,103,1,"Visual Inspection","Air Data Unit",&m_Dlg_Visual_Inspctn,"AN-014-ADU-V00");
    ADD_TEST(204,104,1,"Visual Inspection","Magnetometer",&m_Dlg_Visual_Inspctn,"AN-015-MMS-V00");
    ADD_TEST(205,105,1,"Visual Inspection","AHRS",&m_Dlg_Visual_Inspctn,"1901400000000");
    ADD_TEST(206,106,1,"Visual Inspection","Any Damages",&m_Dlg_Visual_Inspctn,"NO");
    ADD_TEST(207,107,1,"Visual Inspection","Visible Scratches",&m_Dlg_Visual_Inspctn,"NO");
...
#define ADD_TEST(maj, id, majName, minName, dlgPtr, exp) \
    ...
    // -------- VISUAL --------
    ADD_TEST(1, 101, "SMFD FC Visual", "MFD FC Part Number", &dlgSmfdVisual, "Correct");
    ADD_TEST(1, 102, "SMFD FC Visual", "SMFD FC Part Number", &dlgSmfdVisual, "Correct");
    ADD_TEST(1, 103, "SMFD FC Visual", "Damage Check", &dlgSmfdVisual, "OK");
	ADD_TEST(1, 104, "SMFD FC Visual", "Scratches Check", &dlgSmfdVisual, "OK");
	ADD_TEST(1, 105, "SMFD FC Visual", "Screws Check", &dlgSmfdVisual, "OK");
	ADD_TEST(1, 106, "SMFD FC Visual", "Pins Check", &dlgSmfdVisual, "OK");

    // -------- POWER --------
    ADD_TEST(1, 201, "SMFD FC Power", "SMFD FC Current", &dlgSmfdPwr, "< 3A");
    ADD_TEST(1, 202, "SMFD FC Power", "MFD FC Current",  &dlgSmfdPwr, "< 1.8A");


    // -------- SOFTWARE --------
    ADD_TEST(1, 301, "SMFD FC Software", "Operational Software", &dlgSmfdSW, "Correct");


	// -------- ANALOG --------
    ADD_TEST(1, 401, "SMFD FC Analog", "Oil Temp", &dlgSmfdAnalog, "60±2");
    ADD_TEST(1, 402, "SMFD FC Analog", "Oil Pressure", &dlgSmfdAnalog, "40±3");
    ADD_TEST(1, 403, "SMFD FC Analog", "Torque", &dlgSmfdAnalog, "1740±4");
    ADD_TEST(1, 404, "SMFD FC Analog", "Rudder", &dlgSmfdAnalog, "Nose at centre white line");
    ADD_TEST(1, 405, "SMFD FC Analog", "Elevator", &dlgSmfdAnalog, "Nose at green line & -1.2 deg");
    ADD_TEST(1, 406, "SMFD FC Analog", "Aileron", &dlgSmfdAnalog, "Wing at green line");
    ADD_TEST(1, 407, "SMFD FC Analog", "Flap 3V", &dlgSmfdAnalog, "FLAPS 20 in Green");
    ADD_TEST(1, 408, "SMFD FC Analog", "Flap 4V", &dlgSmfdAnalog, "Flaps 30 in Yellow");
    ADD_TEST(1, 409, "SMFD FC Analog", "Generator Volt", &dlgSmfdAnalog, "28±1 V");
    ADD_TEST(1, 410, "SMFD FC Analog", "Battery Volt", &dlgSmfdAnalog, "28±1 V");
    ADD_TEST(1, 411, "SMFD FC Analog", "Current Load", &dlgSmfdAnalog, "200±10 A");

	// -------- ANALOG2 (122–130) --------
ADD_TEST(1, 412, "SMFD FC Analog2", "Hydraulic Main Pressure", &dlgSmfdAnalog2, "2930±20");
ADD_TEST(1, 413, "SMFD FC Analog2", "Hydraulic Emergency Pressure", &dlgSmfdAnalog2, "3330±20");
ADD_TEST(1, 414, "SMFD FC Analog2", "Emergency Brake Pressure", &dlgSmfdAnalog2, "1300±15");
ADD_TEST(1, 415, "SMFD FC Analog2", "LH Brake Pressure", &dlgSmfdAnalog2, "1200±15");
ADD_TEST(1, 416, "SMFD FC Analog2", "RH Brake Pressure", &dlgSmfdAnalog2, "1200±15");
ADD_TEST(1, 417, "SMFD FC Analog2", "Engine Speed", &dlgSmfdAnalog2, "79±2");
ADD_TEST(1, 418, "SMFD FC Analog2", "Fuel LH", &dlgSmfdAnalog2, "168±5");
ADD_TEST(1, 419, "SMFD FC Analog2", "Fuel RH", &dlgSmfdAnalog2, "168±5");
ADD_TEST(1, 420, "SMFD FC Analog2", "Collector Tank Fuel", &dlgSmfdAnalog2, "183±5");

    // -------- FUEL --------
ADD_TEST(1,501, "SMFD FC Fuel Flow Check", "Fuel Flow", &dlgSmfdFuel, "300");

	// -------- DISCRETE (151–158) --------
ADD_TEST(1, 601, "SMFD FC Discrete", "Bus Tie", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 602, "SMFD FC Discrete", "Canopy", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 603, "SMFD FC Discrete", "ECS OFF", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 604, "SMFD FC Discrete", "ECS Over Pressure", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 605, "SMFD FC Discrete", "AOA Mute", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 606, "SMFD FC Discrete", "Alarm Mute", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 607, "SMFD FC Discrete", "Demist", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 608, "SMFD FC Discrete", "Start Switch", &dlgSmfdDiscrete, "OK");

    // -------- REV --------
ADD_TEST(1, 701, "SMFD FC Revisionary Page Check", "Revision Page", &dlgSmfdRev, "Operational");

    

// -------- ADC (131–136) --------
ADD_TEST(1, 801, "SMFD FC ADC", "Altitude (203)", &dlgSmfdADC, "5000");
ADD_TEST(1, 802, "SMFD FC ADC", "Mach (205)", &dlgSmfdADC, "0.5");
ADD_TEST(1, 803, "SMFD FC ADC", "Airspeed (206)", &dlgSmfdADC, "100");
ADD_TEST(1, 804, "SMFD FC ADC", "Altitude Rate (212)", &dlgSmfdADC, "1500");
ADD_TEST(1, 805, "SMFD FC ADC", "SAT (213)", &dlgSmfdADC, "25");
ADD_TEST(1, 806, "SMFD FC ADC", "True Airspeed (210)", &dlgSmfdADC, "100");

// -------- AHRS (137–142) --------
ADD_TEST(1, 901, "SMFD FC AHRS", "Heading (320)", &dlgSmfdAHRS, "60");
ADD_TEST(1, 902, "SMFD FC AHRS", "Pitch (324)", &dlgSmfdAHRS, "20");
ADD_TEST(1, 903, "SMFD FC AHRS", "Roll (325)", &dlgSmfdAHRS, "10");
ADD_TEST(1, 904, "SMFD FC AHRS", "Yaw Rate (330)", &dlgSmfdAHRS, "Valid");
ADD_TEST(1, 905, "SMFD FC AHRS", "Lat Acc (332)", &dlgSmfdAHRS, "Valid");
ADD_TEST(1, 906, "SMFD FC AHRS", "Norm Acc (333)", &dlgSmfdAHRS, "6");

// -------- VOR/ILS (143–147) --------
ADD_TEST(1, 1001, "SMFD FC VOR/ILS", "VOR Frequency", &dlgSmfdVORILS, "108.00");
ADD_TEST(1, 1002, "SMFD FC VOR/ILS", "ILS Frequency", &dlgSmfdVORILS, "108.10");
ADD_TEST(1, 1003, "SMFD FC VOR/ILS", "VOR Bearing", &dlgSmfdVORILS, "90");
ADD_TEST(1, 1004, "SMFD FC VOR/ILS", "Localizer", &dlgSmfdVORILS, "OK");
ADD_TEST(1, 1005, "SMFD FC VOR/ILS", "Glide Slope", &dlgSmfdVORILS, "OK");

// -------- TACAN (148–150) --------
ADD_TEST(1, 1101, "SMFD FC TACAN", "Control Word (145)", &dlgSmfdTACAN, "12X");
ADD_TEST(1, 1102, "SMFD FC TACAN", "Distance (201)", &dlgSmfdTACAN, "300");
ADD_TEST(1, 1103, "SMFD FC TACAN", "Bearing (222)", &dlgSmfdTACAN, "90");


// -------- GPS (159–162) --------
ADD_TEST(1, 1201, "SMFD FC GPS", "Ground Speed",    &dlgSmfdGPS, "1000");
ADD_TEST(1, 1202, "SMFD FC GPS", "True Track",      &dlgSmfdGPS, "90");
ADD_TEST(1, 1203, "SMFD FC GPS", "Magnetic Track",  &dlgSmfdGPS, "90");
ADD_TEST(1, 1204, "SMFD FC GPS", "GPS Time",        &dlgSmfdGPS, "Valid");



#undef ADD_TEST

    iStartTest = DU_ATP_MIN_TEST_ID;
    iEndTest   = DU_ATP_MAX_TEST_ID;
...
int totalTests = sizeof(testOrder) / sizeof(testOrder[0]);

    // -------- EXECUTION LOOP --------
  // -------- EXECUTION LOOP --------
for (int i = 0; i < totalTests; i++)
{
    iCurrentTest = testOrder[i];
    ...
    for (iCurr_Pointer = 0; iCurr_Pointer < iTempCount; iCurr_Pointer++)
    {
        if (iCurrentTest == s_AllTestDetails[iCurr_Pointer].m_uiMinor_Test_ID)
        {
            ...
            {
                // -------- VISUAL --------
                case 101: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 102: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 103: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 104: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 105: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 106: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, TRUE,  pUseFile); break;

                // -------- POWER --------
                case 201: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdPwr, FALSE, pUseFile); break;
                case 202: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdPwr, FALSE, pUseFile); break;
               

                // -------- SOFTWARE --------
                case 301: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdSW, TRUE, pUseFile); break;

                // -------- ANALOG --------
                case 401: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 402: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 403: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 404: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 405: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 406: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 407: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 408: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 409: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 410: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 411: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, TRUE,  pUseFile); break;

                // -------- ANALOG2 --------
                case 412: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 413: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 414: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 415: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 416: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 417: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 418: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 419: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
			    case 420: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, TRUE, pUseFile); break;
              

                // -------- FUEL --------
                case 501: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdFuel, TRUE, pUseFile); break;

                // -------- DISCRETE --------
                case 601: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 602: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 603: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 604: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 605: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 606: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 607: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 608: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, TRUE,  pUseFile); break;

                // -------- REV --------
                case 701: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdRev, TRUE, pUseFile); break;

                // -------- ADC --------
                case 801: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 802: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 803: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 804: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 805: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 806: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, TRUE,  pUseFile); break;

                // -------- AHRS --------
                case 901: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 902: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 903: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 904: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 905: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 906: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, TRUE,  pUseFile); break;

                // -------- VOR/ILS --------
                case 1001: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1002: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1003: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1004: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1005: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, TRUE,  pUseFile); break;

                // -------- TACAN --------
                case 1101: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdTACAN, FALSE, pUseFile); break;
                case 1102: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdTACAN, FALSE, pUseFile); break;
                case 1103: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdTACAN, TRUE,  pUseFile); break;

				// -------- GPS --------
                case 1201: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, FALSE, pUseFile);break;
                case 1202:ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, FALSE, pUseFile); break;
                case 1203: ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, FALSE, pUseFile);break;
                case 1204:ProcessTest(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, TRUE, pUseFile); break;
...
void CSIPUPowerONPage::ExecuteSMFDSequenceRC(CFile* pFile)
{
    ...
    CDialog_Smfd_Analog_RC dlgSmfdAnalog;
    CDialog_Smfd_Fuel_RC   dlgSmfdFuel;
    CDialog_Smfd_Rev_RC dlgSmfdRev;
	CDialog_Smfd_Analog1_RC dlgSmfdAnalog2;
...
#define ADD_TEST(maj, id, majName, minName, dlgPtr, exp) \
    ...
    // -------- VISUAL --------
    ADD_TEST(1, 101, "SMFD Visual", "MFD Part Number", &dlgSmfdVisual, "Correct");
    ADD_TEST(1, 102, "SMFD Visual", "SMFD Part Number", &dlgSmfdVisual, "Correct");
    ADD_TEST(1, 103, "SMFD Visual", "Damage Check", &dlgSmfdVisual, "OK");
	ADD_TEST(1, 104, "SMFD Visual", "Scratches Check", &dlgSmfdVisual, "OK");
	ADD_TEST(1, 105, "SMFD Visual", "Screws Check", &dlgSmfdVisual, "OK");
	ADD_TEST(1, 106, "SMFD Visual", "Pins Check", &dlgSmfdVisual, "OK");

    // -------- POWER --------
    ADD_TEST(1, 201, "SMFD Power", "SMFD1 Current", &dlgSmfdPwr, "< 3A");
 
    ADD_TEST(1, 203, "SMFD Power", "MFD1 Current",  &dlgSmfdPwr, "< 1.8A");

    // -------- SOFTWARE --------
    ADD_TEST(1, 301, "SMFD Software", "Operational Software", &dlgSmfdSW, "Correct");


	// -------- ANALOG --------
    ADD_TEST(1, 401, "SMFD Analog", "Oil Temp", &dlgSmfdAnalog, "60±2");
    ADD_TEST(1, 402, "SMFD Analog", "Oil Pressure", &dlgSmfdAnalog, "40±3");
    ADD_TEST(1, 403, "SMFD Analog", "Torque", &dlgSmfdAnalog, "1740±4");
    ADD_TEST(1, 404, "SMFD Analog", "Rudder", &dlgSmfdAnalog, "Nose at centre white line");
    ADD_TEST(1, 405, "SMFD Analog", "Elevator", &dlgSmfdAnalog, "Nose at green line & -1.2 deg");
    ADD_TEST(1, 406, "SMFD Analog", "Aileron", &dlgSmfdAnalog, "Wing at green line");
    ADD_TEST(1, 407, "SMFD Analog", "Flap 3V", &dlgSmfdAnalog, "FLAPS 20 in Green");
    ADD_TEST(1, 408, "SMFD Analog", "Flap 4V", &dlgSmfdAnalog, "Flaps 30 in Yellow");
    ADD_TEST(1, 409, "SMFD Analog", "Generator Volt", &dlgSmfdAnalog, "28±1 V");
    ADD_TEST(1, 410, "SMFD Analog", "Battery Volt", &dlgSmfdAnalog, "28±1 V");
    ADD_TEST(1, 411, "SMFD Analog", "Current Load", &dlgSmfdAnalog, "200±10 A");

	// -------- ANALOG2 (122–130) --------
ADD_TEST(1, 412, "SMFD Analog2", "Hydraulic Main Pressure", &dlgSmfdAnalog2, "2930±20");
ADD_TEST(1, 413, "SMFD Analog2", "Hydraulic Emergency Pressure", &dlgSmfdAnalog2, "3330±20");
ADD_TEST(1, 414, "SMFD Analog2", "Emergency Brake Pressure", &dlgSmfdAnalog2, "1300±15");
ADD_TEST(1, 415, "SMFD Analog2", "LH Brake Pressure", &dlgSmfdAnalog2, "1200±15");
ADD_TEST(1, 416, "SMFD Analog2", "RH Brake Pressure", &dlgSmfdAnalog2, "1200±15");
ADD_TEST(1, 417, "SMFD Analog2", "Engine Speed", &dlgSmfdAnalog2, "79±2");
ADD_TEST(1, 418, "SMFD Analog2", "Fuel LH", &dlgSmfdAnalog2, "168±5");
ADD_TEST(1, 419, "SMFD Analog2", "Fuel RH", &dlgSmfdAnalog2, "168±5");
ADD_TEST(1, 420, "SMFD Analog2", "Collector Tank Fuel", &dlgSmfdAnalog2, "183±5");

    // -------- FUEL --------
ADD_TEST(1,501, "SMFD Fuel Flow Check", "Fuel Flow", &dlgSmfdFuel, "300");

	// -------- DISCRETE (151–158) --------
ADD_TEST(1, 601, "SMFD Discrete", "Bus Tie", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 602, "SMFD Discrete", "Canopy", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 603, "SMFD Discrete", "ECS OFF", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 604, "SMFD Discrete", "ECS Over Pressure", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 605, "SMFD Discrete", "AOA Mute", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 606, "SMFD Discrete", "Alarm Mute", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 607, "SMFD Discrete", "Demist", &dlgSmfdDiscrete, "OK");
ADD_TEST(1, 608, "SMFD Discrete", "Start Switch", &dlgSmfdDiscrete, "OK");

    // -------- REV --------
ADD_TEST(1, 701, "SMFD Revisionary Page Check", "Revision Page", &dlgSmfdRev, "Operational");

    

// -------- ADC (131–136) --------
ADD_TEST(1, 801, "SMFD ADC", "Altitude (203)", &dlgSmfdADC, "5000");
ADD_TEST(1, 802, "SMFD ADC", "Mach (205)", &dlgSmfdADC, "0.5");
ADD_TEST(1, 803, "SMFD ADC", "Airspeed (206)", &dlgSmfdADC, "100");
ADD_TEST(1, 804, "SMFD ADC", "Altitude Rate (212)", &dlgSmfdADC, "1500");
ADD_TEST(1, 805, "SMFD ADC", "SAT (213)", &dlgSmfdADC, "25");
ADD_TEST(1, 806, "SMFD ADC", "True Airspeed (210)", &dlgSmfdADC, "100");

// -------- AHRS (137–142) --------
ADD_TEST(1, 901, "SMFD AHRS", "Heading (320)", &dlgSmfdAHRS, "60");
ADD_TEST(1, 902, "SMFD AHRS", "Pitch (324)", &dlgSmfdAHRS, "20");
ADD_TEST(1, 903, "SMFD AHRS", "Roll (325)", &dlgSmfdAHRS, "10");
ADD_TEST(1, 904, "SMFD AHRS", "Yaw Rate (330)", &dlgSmfdAHRS, "Valid");
ADD_TEST(1, 905, "SMFD AHRS", "Lat Acc (332)", &dlgSmfdAHRS, "Valid");
ADD_TEST(1, 906, "SMFD AHRS", "Norm Acc (333)", &dlgSmfdAHRS, "6");

// -------- VOR/ILS (143–147) --------
ADD_TEST(1, 1001, "SMFD VOR/ILS", "VOR Frequency", &dlgSmfdVORILS, "108.00");
ADD_TEST(1, 1002, "SMFD VOR/ILS", "ILS Frequency", &dlgSmfdVORILS, "108.10");
ADD_TEST(1, 1003, "SMFD VOR/ILS", "VOR Bearing", &dlgSmfdVORILS, "90");
ADD_TEST(1, 1004, "SMFD VOR/ILS", "Localizer", &dlgSmfdVORILS, "OK");
ADD_TEST(1, 1005, "SMFD VOR/ILS", "Glide Slope", &dlgSmfdVORILS, "OK");

// -------- TACAN (148–150) --------
ADD_TEST(1, 1101, "SMFD TACAN", "Control Word (145)", &dlgSmfdTACAN, "12X");
ADD_TEST(1, 1102, "SMFD TACAN", "Distance (201)", &dlgSmfdTACAN, "300");
ADD_TEST(1, 1103, "SMFD TACAN", "Bearing (222)", &dlgSmfdTACAN, "90");


// -------- GPS (159–162) --------
ADD_TEST(1, 1201, "SMFD GPS", "Ground Speed",    &dlgSmfdGPS, "1000");
ADD_TEST(1, 1202, "SMFD GPS", "True Track",      &dlgSmfdGPS, "90");
ADD_TEST(1, 1203, "SMFD GPS", "Magnetic Track",  &dlgSmfdGPS, "90");
ADD_TEST(1, 1204, "SMFD GPS", "GPS Time",        &dlgSmfdGPS, "Valid");
...
for (int i = 0; i < totalTests; i++)
{
    ...
    for (iCurr_Pointer = 0; iCurr_Pointer < iTempCount; iCurr_Pointer++)
    {
        if (iCurrentTest == s_AllTestDetails[iCurr_Pointer].m_uiMinor_Test_ID)
        {
            ...
            {
                // -------- VISUAL --------
                case 101: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 102: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 103: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 104: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 105: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, FALSE, pUseFile); break;
                case 106: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVisual, TRUE,  pUseFile); break;

                // -------- POWER --------
                case 201: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdPwr, FALSE, pUseFile); break;
                case 202: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdPwr, FALSE, pUseFile); break;
                case 203: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdPwr, FALSE, pUseFile); break;
                case 204: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdPwr, TRUE,  pUseFile); break;

                // -------- SOFTWARE --------
                case 301: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdSW, TRUE, pUseFile); break;

                // -------- ANALOG --------
                case 401: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 402: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 403: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 404: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 405: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 406: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 407: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 408: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 409: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 410: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, FALSE, pUseFile); break;
                case 411: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog, TRUE,  pUseFile); break;

                // -------- ANALOG2 --------
                case 412: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 413: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 414: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 415: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 416: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 417: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 418: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
                case 419: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, FALSE, pUseFile); break;
			    case 420: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAnalog2, TRUE, pUseFile); break;
              

                // -------- FUEL --------
                case 501: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdFuel, TRUE, pUseFile); break;

                // -------- DISCRETE --------
                case 601: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 602: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 603: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 604: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 605: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 606: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 607: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, FALSE, pUseFile); break;
                case 608: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdDiscrete, TRUE,  pUseFile); break;

                // -------- REV --------
                case 701: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdRev, TRUE, pUseFile); break;

                // -------- ADC --------
                case 801: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 802: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 803: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 804: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 805: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, FALSE, pUseFile); break;
                case 806: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdADC, TRUE,  pUseFile); break;

                // -------- AHRS --------
                case 901: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 902: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 903: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 904: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 905: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, FALSE, pUseFile); break;
                case 906: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdAHRS, TRUE,  pUseFile); break;

                // -------- VOR/ILS --------
                case 1001: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1002: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1003: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1004: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, FALSE, pUseFile); break;
                case 1005: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdVORILS, TRUE,  pUseFile); break;

                // -------- TACAN --------
                case 1101: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdTACAN, FALSE, pUseFile); break;
                case 1102: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdTACAN, FALSE, pUseFile); break;
                case 1103: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdTACAN, TRUE,  pUseFile); break;

				// -------- GPS --------
                case 1201: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, FALSE, pUseFile);break;
                case 1202:ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, FALSE, pUseFile); break;
                case 1203: ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, FALSE, pUseFile);break;
                case 1204:ProcessTestRC(iCurrentTest, bDlgFlag, iResult, &dlgSmfdGPS, TRUE, pUseFile); break;
...
{
    CDialog_Smfd_Pwr_RC* dlg = (CDialog_Smfd_Pwr_RC*)pDlg;
    measured = dlg->m_strSMFD2Current;
    status   = dlg->m_strStatusSMFD2Current;
    break;
}
case 203:
{
...
{
    CDialog_Smfd_Pwr_RC* dlg = (CDialog_Smfd_Pwr_RC*)pDlg;
    measured = dlg->m_strMFD2Current;
    status   = dlg->m_strStatusMFD2Current;
    break;
}
...
case 401:
{
    CDialog_Smfd_Analog_RC* dlg = (CDialog_Smfd_Analog_RC*)pDlg;
    measured = dlg->m_stroiltemp;
    status   = dlg->m_strStatusoiltemp;
    break;
}
...
case 412:
{
    CDialog_Smfd_Analog1_RC* dlg = (CDialog_Smfd_Analog1_RC*)pDlg;
    measured = dlg->m_strHydPresMain;
    status   = dlg->m_strStatusHydPresMain;
    break;
}
...
ADD_TEST(302,102,1,"ADC Visual Inspection","Damage Check",&dlgVisual,"NO");
ADD_TEST(303,103,1,"ADC Visual Inspection","Scratches",&dlgVisual,"NO");
ADD_TEST(304,104,1,"ADC Visual Inspection","Screws",&dlgVisual,"NO");
ADD_TEST(305,105,1,"ADC Visual Inspection","Pins",&dlgVisual,"NO");
ADD_TEST(306,106,1,"ADC Visual Inspection","Marking",&dlgVisual,"YES");
    ...
    if(iCurrentTest%100<=DU_ATP_MAX_NO_OF_SUBTEST)
    {
            ...
            iCurr_Pointer++)
        {
                ...
                s_AllTestDetails[iCurr_Pointer].m_uiMinor_Test_ID)
            {
                ...
                {
                    iResult=
                        s_AllTestDetails[iCurr_Pointer]
                        .m_Procedure_Dlg
                        ->DoModal();
...
#define ADD_TEST(maj, id, majName, minName, dlgPtr, exp) \
	...
	// -------- VISUAL (501–502) --------
	ADD_TEST(5,501,"GPS Visual Inspection","Part Number",&dlgGPSVisual,"011-04629-00");
	ADD_TEST(5,502,"GPS Visual Inspection","Damage Check",&dlgGPSVisual,"OK");
	ADD_TEST(5,503,"GPS Visual Inspection","Scratches Check",&dlgGPSVisual,"OK");
	ADD_TEST(5,504,"GPS Visual Inspection","Screws Check",&dlgGPSVisual,"OK");
	ADD_TEST(5,505,"GPS Visual Inspection","Bend Pins Check",&dlgGPSVisual,"OK");


	
	ADD_TEST(5,506,"GPS Power Consumption","Voltage Check",&dlgGPSPWR,"28V DC ± 1V");
	ADD_TEST(5,507,"GPS Power Consumption","Current Check",&dlgGPSPWR,"1 ± 0.7A");
	//ADD_TEST(5,508,"GPS Signal Acquisition","GPS Signal Check ",&dlgGPSSignal,"Correct");
	ADD_TEST(5,508,"GPS DataBase Info","Navigation Check ",&dlgGPSDbInfo,"Available");
	ADD_TEST(5,509,"GPS DataBase Info","BaseMap Check ",&dlgGPSDbInfo,"Available");
	ADD_TEST(5,510,"GPS DataBase Info","Obstacle/Hot line Check ",&dlgGPSDbInfo,"Available");
	ADD_TEST(5,511,"GPS DataBase Info","Safe Taxi Check ",&dlgGPSDbInfo,"Available");
	ADD_TEST(5,512,"GPS DataBase Info","Terrain Check ",&dlgGPSDbInfo,"Available");
	ADD_TEST(5,513,"GPS System","Serial Number",&dlgGPSSysInfo,"Correct");
	ADD_TEST(5,514,"GPS System","System ID",&dlgGPSSysInfo,"Correct");
	ADD_TEST(5,515,"GPS System","Main SW",&dlgGPSSysInfo,"Correct");
	ADD_TEST(5,516,"GPS System","GPS/WAAS SW",&dlgGPSSysInfo,"Correct");



	//ADD_TEST(5,516,"GPS Time","GPS Time",NULL,"Correct");
	//ADD_TEST(5,517,"GPS Time","GPS Date",NULL,"Correct");
		...
		if (iCurrentTest % 100 <= DU_ATP_MAX_NO_OF_SUBTEST)
		{
			for (iCurr_Pointer = 0; iCurr_Pointer < iTempCount; iCurr_Pointer++)
			{
				if (iCurrentTest == s_AllTestDetails[iCurr_Pointer].m_uiMinor_Test_ID)
				{
					...
					// -------- GROUP 5 (GPS) --------
					if ((iCurrentTest / 100) == 5)
					{
						if (!bDlgFlag && s_AllTestDetails[iCurr_Pointer].m_Procedure_Dlg != NULL)
						{
							iResult = s_AllTestDetails[iCurr_Pointer].m_Procedure_Dlg->DoModal();
							bDlgFlag = TRUE;
						}
						...
						case 502:
							...
							break;
                        case 503:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgGPSVisual.m_strGPSScratches);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgGPSVisual.m_strStatusGPSScratches);
							break;
                        case 504:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgGPSVisual.m_strGPSScrews);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgGPSVisual.m_strStatusGPSScrews);
							break;
						case 505:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgGPSVisual.m_strGPSPins);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgGPSVisual.m_strStatusGPSPins);
							bDlgFlag = FALSE;
							break;
						case 506:
...
#define ADD_TEST(maj, id, majName, minName, dlgPtr, exp) \
	...
	ADD_TEST(6, 601, "VOR/ILS TACAN Visual", "VOR/ILS Receiver", &dlgVORTAC_Visual, "ANS2810A HTT1");
	ADD_TEST(6, 602, "VOR/ILS TACAN Visual", "TACAN Receiver", &dlgVORTAC_Visual, "2920A HTT1");
	ADD_TEST(6, 603, "VOR/ILS TACAN Visual", "CDU", &dlgVORTAC_Visual, "CDU-2810A HTT1");
	ADD_TEST(6, 604, "VOR/ILS TACAN Visual", "Damage Check", &dlgVORTAC_Visual, "Satisfactory");
	ADD_TEST(6, 605, "VOR/ILS TACAN POWER", "Power Consumption", &dlgVORTAC_PWR, "< 3 Amps");
			...
			for (iCurr_Pointer = 0; iCurr_Pointer < iTempCount; iCurr_Pointer++)
			{
				if (iCurrentTest == s_AllTestDetails[iCurr_Pointer].m_uiMinor_Test_ID)
				{
					...
					if ((iCurrentTest / 100) == 6)
					{
						...
						{
						case 601:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgVORTAC_Visual.m_strVOR);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgVORTAC_Visual.m_strStatusVOR);
							break;

						case 602:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgVORTAC_Visual.m_strTACAN);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgVORTAC_Visual.m_strStatusTACAN);
							break;

						case 603:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgVORTAC_Visual.m_strCDU);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgVORTAC_Visual.m_strStatusCDU);
							break;

						case 604:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgVORTAC_Visual.m_strDamage);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgVORTAC_Visual.m_strStatusDamage);
							bDlgFlag = FALSE;
							break;
					   case 605:
							strcpy(s_Curr_Test_Result.m_strMeasuered_Val, dlgVORTAC_PWR.m_strVORTACCurrent);
							strcpy(s_Curr_Test_Result.m_strTestStatus, dlgVORTAC_PWR.m_strStatusVORTACCurrent);
							bDlgFlag = FALSE;
							break;
						}
					}
