...
////////////////////////////////////////////////////////////////////////////
// CXTPagerCtrl is a CWnd derived class which is used to contain and scroll 
// another window, and wraps the windows pager API.

class CXTPagerCtrl : public CWnd
{
    DECLARE_DYNAMIC(CXTPagerCtrl)

public:
    
    // -> Remarks:  Constructs a CXTPagerCtrl object.
    CXTPagerCtrl();

    // -> Remarks:  Destroys a CXTPagerCtrl object, handles cleanup and de-allocation.
...
public:
	...
	// -> Input:   dwStyle    - Specifies the window style attributes. The following styles 
	//                          are also used when creating pager controls, and can be 
	//                          combined by using the or operator:
	//
	//                          PGS_AUTOSCROLL - The pager control will scroll when the 
	//                                           user hovers the mouse over one of the 
	//                                           scroll buttons.
    //                          PGS_DRAGNDROP  - The contained window can be a drag-and-drop
	//                                           target. The pager control will automatically
	//                                           scroll if an item is dragged from outside the
	//                                           pager over one of the scroll buttons.
	//                          PGS_HORZ       - Creates a pager control that can be scrolled
	//                                           horizontally. This style and the PGS_VERT 
	//                                           style are mutually exclusive and cannot be 
	//                                           combined. 
    //                          PGS_VERT       - Creates a pager control that can be scrolled 
	//                                           vertically. This is the default direction if 
	//                                           no direction style is specified. This style 
	//                                           and the PGS_HORZ style are mutually exclusive
	...
	// -> Remarks: Creates a pager child window and attaches it to this CWnd object.
    virtual BOOL Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID);

    // -> Input:   hwndChild - Handle to the window to be contained.
    // -> Remarks: Sets the contained window for the pager control. This command will 
	//             not change the parent of the contained window; it only assigns a 
	//             window handle to the pager control for scrolling. In most cases, the 
	//             contained window will be a child window. If this is the case, the 
	//             contained window should be a child of the pager control.
    void SetChild(HWND hwndChild);

    // -> Remarks: Forces the pager control to recalculate the size of the 
	//             contained window. Using this command will result in a PGN_CALCSIZE 
	//             notification being sent.
    void RecalcSize();

    // -> Input:   bForward - BOOL value that determines if mouse forwarding is enabled 
	//                        or disabled. If this value is nonzero, mouse forwarding is 
	//                        enabled. If this value is zero, mouse forwarding is disabled.
    // -> Remarks: Enables or disables mouse forwarding for the pager control. When 
	//             mouse forwarding is enabled, the pager control forwards WM_MOUSEMOVE 
	//             messages to the contained window.
    void ForwardMouse(BOOL bForward);

    // -> Input:   clr - COLORREF value that contains the new background color 
    //                   of the pager control.
    // -> Returns: Returns a COLORREF value that contains the previous background color.     
    // -> Remarks: Sets the current background color for the pager control.
    COLORREF SetBkColor(COLORREF clr);

    // -> Returns: Returns a COLORREF value that contains the current background color.
	...
	//                       to zero.
    // -> Returns: Returns an INT value that contains the previous border size, in pixels.
    // -> Remarks: Sets the current border size for the pager control.
    int SetBorder(int iBorder);

    // -> Returns: Returns an INT value that contains the current border size, in pixels.
    // -> Remarks: Retrieves the current border size for the pager control.
    int GetBorder();

    // -> Input:   iPos - INT value that contains the new scroll position, in pixels.
    // -> Remarks: Sets the scroll position for the pager control.
    void SetPos(int iPos);

    // -> Returns: Returns an INT value that contains the current scroll position, in pixels.
    // -> Remarks: Retrieves the current scroll position of the pager control.
    int GetPos();

    // -> Input:   iSize - INT value that contains the new button size, in pixels.
    // -> Returns: Returns an INT value that contains the previous button size, in pixels.
    // -> Remarks: Sets the current button size for the pager control.
    int SetButtonSize(int iSize);

    // -> Returns: Returns an INT value that contains the current button size, in pixels.
    // -> Remarks: Retrieves the current button size for the pager control.
    int GetButtonSize();

    // -> Input:   iButton - Indicates which button to retrieve the state for. See the 
	//                       description for iButton in PGM_GETBUTTONSTATE for a list of 
	//                       possible values.
    // -> Returns: Returns the state of the button specified in iButton. See the return 
	//             value description in PGM_GETBUTTONSTATE for a list of possible values.     
	...
	//                    pointer. It is the caller's responsibility to call Release on 
